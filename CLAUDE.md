...
### Fitting
`scipy.optimize.curve_fit` with bounds, `jac=<analytical>`, and `x_scale='jac'`. When calibration is on, `fit_spectrum(..., fit_mask=...)` drops the lamp-line regions (±8 cm⁻¹) from the fitted points so their huge unmodelled residuals can't dominate the least-squares cost or pull broad spurious peaks into the gaps between bright lamp lines (the model is still evaluated on the full axis for plotting/residuals). Initial guesses: amplitude from local peak height, centre from local peak position, FWHM from 5× median point spacing, eta=0.5. Bounds allow centres to shift ±15×dx and FWHM up to 150×dx. maxfev=30000.
