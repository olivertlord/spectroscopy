...
- `cal_position_sigma(cal, x)` — 1σ calibration uncertainty at wavenumber(s) x, `sigma·sqrt(v(x)·invVtV·v(x))` in scaled coords. This systematic is propagated into the parameter tables (separate `Centre err (cal)` column alongside `Centre err (fit)`, plus a `Centre err (total)` = `hypot(fit, cal)` convenience column) and folded into the summary plot's centre-axis error bars in quadrature. The three are kept distinct on purpose: `fit` is random/per-peak, `cal` is systematic and common-mode within a spectrum (so it cancels in same-spectrum peak differences — use `fit` there), and `total` is the quadrature sum to quote for an absolute position or a cross-spectrum comparison. The multi-spectrum export TSV carries all three as `peakN_centre_err_fit` / `_cal` / `_total`.
