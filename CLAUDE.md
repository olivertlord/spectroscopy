...
- *Shared-η couples everything.* `Pseudo-Voigt (shared η)` fits one global η for all peaks, so changing the peak set (more files, or a cropped window) changes η and hence *every* peak's FWHM — even isolated ones. Plain Pseudo-Voigt avoids this; prefer it when per-peak FWHM/position stability matters. A possible future enhancement (not yet implemented) is a per-file local-support gate that excludes union peaks with no evidence in a given spectrum *before* fitting, so absent neighbours can't perturb real bands — it trades a little transect sensitivity for batch/window reproducibility and should be an opt-in.
